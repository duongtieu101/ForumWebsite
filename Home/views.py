...
def index(request):
    Data= {'Boards1':Board.objects.all()[:3],
    ...
    'Threads':Thread.objects.all().order_by("-Time_Created")[:10], 'Users':User.objects.all()}
...
def index_board(request, id):
        ...
        return render(request,"Home/index_board.html",Data)
...
def index_thread(request, id):       
        ...
        return render(request, "Home/index_thread.html", Data)
...
def index_user(request, username):
    ...
    return render(request,"Home/index_user.html", Data)

def index_danhsachbaidang(request):
        ...
        return render(request,"Home/index_danhsachbaidang.html", Data)

def index_danhsachthanhvien(request):
    Data={'User':User.objects.all().order_by("-date_joined"), 'Threads':Thread.objects.all().order_by("-Time_Created")[:10]}
    return render(request,"Home/index_danhsachthanhvien.html", Data)

def dieukhoan_quydinh(request):
        Data={'Threads':Thread.objects.all().order_by("-Time_Created")[:10]}
        return render(request,"Home/dieukhoan_quydinh.html",Data)
...
def register(request):
    ...
    return render(request, "Home/register.html", {'form':form,'Threads':Thread.objects.all().order_by("-Time_Created")[:10]})
